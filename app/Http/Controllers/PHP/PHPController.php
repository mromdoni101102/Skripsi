...
class PHPController extends Controller{
    public function submit_score_baru(Request $request)
    {
        ...
        DB::table('php_user_submits')->insert([
            'user_id' => $userId,
            'score' => $score,
            'topics_id' => $topicsId,
        ...
        $currentRank = DB::table('php_student_rank')
        ...
        if ($score > 50 ) {
            $exists = DB::table('php_student_enroll')
            ...
            if (!$exists) {
                ...
                if ( $flags == 0) {
                    DB::table('php_student_enroll')->insert([
    ...
    public function index(){
        ...
        $actual = "";
        $topics = Topic::all();
        $topics_detail = Topic_detail::all();
       ...
       if ($roleTeacher[0]->role == "student") {
            $completedTopics = DB::table('php_student_enroll')
                ->join('users', 'php_student_enroll.id_users', '=', 'users.id')
                ->join('php_topics_detail', 'php_student_enroll.php_topics_detail_id', '=', 'php_topics_detail.id')
                ->select('php_student_enroll.*', 'users.name as user_name', 'php_topics_detail.*')
                ->where('php_student_enroll.id_users', $idUser)
                ->get();
            ...
            $completedTopics = collect(); // Return an empty collection if role is neither student nor teacher
        }
        ...
        foreach ($completedTopics as $enrollment) {
            ...
            if (!isset($progress[$userId])) {
                $userCompletedCount = DB::table('php_student_enroll')
...
//        $listTask = DB::select("select aa.*, us.name from php_user_submits aa join users us on aa.userid = us.id where php_id = $phpid and php_id_topic = $start ");
        ...
        $exists = DB::table('php_student_enroll')
            ->where('id_users', Auth::user()->id)
            ->where('php_topics_detail_id', $start)
            ->exists();
        ...
        if (!$exists) {
            ...
            if ( $flags == 0) {
                DB::table('php_student_enroll')->insert([
                    'id_users' => Auth::user()->id,
                    'php_topics_detail_id' => $start,
                    'created_at' => now()
                ]);
            }

        }
        $completedTopics = UserEnroll::where('id_users',  Auth::user()->id)->distinct('php_topics_detail_id')->count();
